<template>
  <div>
    ...
    <div class="card-body card">
      <RequestsTable :headers="headers" :data="Users.OrderList"></RequestsTable>
    </div>
  </div>
  <div class="card-fillter">
    <SheetsDownload :headers="headers" :data="Users.OrderList" sheetName="Sheet List" v-if="Users.OrderList != 0" />
  </div>
</template>
<script setup>
import SheetsDownload from "@/components/global/SheetsDownload.vue";
import Table from "@/components/pages/Orders/table.vue";
...
const MakeSortingNew = () => {
  ...
  sortingUserNew.value =Users.OrderList.sort((a, b) => {
    return new Date(a.created_at) - new Date(b.created_at);
  });
 Users.OrderList = [];
 Users.OrderList = sortingUserNew.value;
 showDropdown.value = false;
}
const MakeSortingLast = () => {
  const sortingUserLast = ref([]);
  sortingUserLast.value =Users.OrderList.sort((a, b) => {
    return new Date(b.created_at) - new Date(a.created_at);
  });
  console.log("sortingUserLast =", sortingUserLast.value);
 Users.OrderList = [];
 Users.OrderList = sortingUserLast.value;
 showDropdown.value = false;
