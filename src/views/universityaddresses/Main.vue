<template>
  <div class="card px-5 card-body py-3 mt-5">
    ...
    <div class=" mb-3">
        <button
          class="addTitle"
          @click="showpopupAddTitle = true"
          type="button"
        >
          اضافة عنوان
        </button>
      </div>
    <div v-if="!Trips.sourceDestinationend_pointLoading">
      <div
        class="content-flex mb-3"
        v-for="end_point in Trips.sourceDestinationend_point"
        :key="end_point"
      >
        <img src="@/assets/images/icon_location.svg" class="mt-3" />
        <div class="input-group">
          <label class="mb-2">عنوان الجامعة</label>
          <div class="content-text-input">{{ end_point.name_ar }}</div>
        </div>
        ...
        <div class="Dropdown Dropdown-action">
          ...
          <ul>
            <li v-if="!end_point.is_active"> <span class="make-active"
                @click="checkActive(end_point.is_active, end_point.id)">
                تفعيل
              </span>
            </li>
            <li v-else>
              <span class="make-notactive" @click="checkNoActive(end_point.is_active, end_point.id)">
                تعطيل
              </span>
            </li>
            <li>
              <span @click="openUpdat(end_point.id, end_point.name_ar)"><img
                  src="@/assets/images/edit-button-svgrepo-com.svg">
                تعديل
              </span>
            </li>
            <li>
              <span class="make-notactive" @click="DeleteFun(end_point.id)"><img src="@/assets/images/icons-delete.svg" >حذف</span>
        ...
        <!-- <div class="d-flex gap-3 actions mt-4 pt-2">
          <span
            class="make-active"
            :class="end_point.is_active ? 'make-active' : 'make-cancel'"
            @click="checkActive(end_point.is_active, end_point.id)"
          >
            تفعيل
          </span>
          <span
            :class="!end_point.is_active ? 'make-notactive' : 'make-cancel'"
            @click="checkNoActive(end_point.is_active, end_point.id)"
          >
            تعطيل
          </span>
          <span
            class="make-cancel"
            @click="openUpdat(end_point.id, end_point.name_ar)"
          >
            تعديل
          </span>
        </div> -->
    ...
    <div
      v-if="Trips.sourceDestinationend_pointLoading"
    ...
    <popupActive v-if="showpopupActive">
      <template #body
        ><h2 class="text-center py-5">
          هل انت متأكد من إعادة تفعيل عنوان الجامعة؟
        </h2>
        <div class="text-center gap-2 d-flex buttons m-auto">
          <button class="send" @click="FunChage(1)">نعم ،أرغب</button
    ...
    <popupNotActive v-if="showpopupNoActive">
      <template #body
        ><h2 class="text-center py-5">
          هل انت متأكد من تعطيل العنوان الجامعة؟
        </h2>
        <div class="text-center gap-2 d-flex buttons m-auto">
          <button class="send" @click="FunChage(0)">نعم ،أرغب</button
          ><button class="cancel" @click="showpopupNoActive = false">
            لا ، ارغب
          </button>
        </div></template
      >
    </popupNotActive>
    <popupEdit v-if="showpopupEdit" class="popupEdit"
      ><template #header>تعديل عنوان الجامعة </template>
      <template #body>
        <form @submit.prevent="onSubmit">
          ...
          <div class="text-center buttons">
            ...
            ><button class="cancel" @click="showpopupEdit = false">
              إلغاء
            </button>
          </div>
        </form></template
    ...
    <popupAddTitle v-if="showpopupAddTitle" class="popupEdit"
      ><template #header>إضافة عنوان الجامعة </template>
      <template #body
        ><form>
          <div class="mb-3">
            <label for="exampleFormControlInput1" class="form-label"
          ...
          <div class="text-center buttons">
            <button class="send" @click="addNew" type="button">حفظ</button
            ><button
              class="cancel"
              @click="showpopupAddTitle = false"
              type="button"
            >
              إلغاء
            </button>
...
const FunChage = async (val) => {
  ...
  await Trips.ChangeStatusPoint({ point_id: point_id.value, is_active: val });
};
...
const DeleteFun = async(id) => {
    await Trips.deletesource(id,"end_point");
};
onMounted(async () => {
  if (Trips.sourceDestinationend_point.length == 0) {
